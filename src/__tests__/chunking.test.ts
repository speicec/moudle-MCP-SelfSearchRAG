...
describe('SmallToBigRetriever', () => {
  // Helper to create 1536-dim embeddings (matches text-embedding-3-small and syntheticEmbedding)
  ...
  // Helper to create 1536-dim embeddings (matches text-embedding-3-small)
  function createRealisticEmbedding(seed: number): number[] {
    const embedding: number[] = [];
    for (let i = 0; i < 1536; i++) {
      embedding.push((Math.sin(seed + i * 0.1) + 1) / 2);
    }
    // Normalize
    const norm = Math.sqrt(embedding.reduce((sum, v) => sum + v * v, 0));
    if (norm > 0) {
  ...
  // 6.2: Setup store with realistic 1536-dim embeddings for real embedding tests
  async function setupStoreWithRealisticEmbeddings(): Promise<HierarchicalStore> {
    const store = createHierarchicalStore({
      parentChunkMinTokens: 50,
      parentChunkMaxTokens: 200,
    });
  ...
  // 6.1: Unit tests for embedding generator configuration
  describe('setEmbeddingGenerator', () => {
    it('should accept embedding generator function', async () => {
      const store = await setupStoreWithChunks();
      const retriever = createSmallToBigRetriever(store);

      const mockGenerator = async (text: string) => createEmbedding([1, 0, 0]);
      retriever.setEmbeddingGenerator(mockGenerator);

      // Should use generator for query embedding
      const embedding = await retriever.getQueryEmbedding('test');
      expect(embedding).toEqual(createEmbedding([1, 0, 0]));
    });

    it('should use generator for similarity calculation', async () => {
      const store = await setupStoreWithChunks();
      const retriever = createSmallToBigRetriever(store, {
      ...
      // Mock generator that produces embeddings similar to stored chunks
      retriever.setEmbeddingGenerator(async (text: string) => {
        if (text.includes('machine learning')) {
          return createEmbedding([1, 0, 0, 0, 0]);
        }
        return createEmbedding([0.5, 0.5, 0, 0, 0]);
      });

      const results = await retriever.retrieve('machine learning');

      expect(results.length).toBeGreaterThan(0);
      // First result should be machine learning chunk (highest similarity)
      expect(results[0]?.smallChunkContent).toContain('Machine learning');
    });

    it('should cache embeddings generated by generator', async () => {
      const store = await setupStoreWithChunks();
      const retriever = createSmallToBigRetriever(store);

      let callCount = 0;
      const mockGenerator = async (text: string) => {
        callCount++;
        return createEmbedding([1, 0, 0]);
      };
      retriever.setEmbeddingGenerator(mockGenerator);

      // Call twice with same query
      await retriever.getQueryEmbedding('cached query');
      await retriever.getQueryEmbedding('cached query');
  ...
  describe('error handling', () => {
    it('should throw error when embedding generator fails', async () => {
      const store = await setupStoreWithChunks();
      const retriever = createSmallToBigRetriever(store);

      // Mock generator that throws error
      retriever.setEmbeddingGenerator(async () => {
        throw new Error('Embedding API failed');
      });

      await expect(retriever.getQueryEmbedding('test query')).rejects.toThrow('Embedding API failed');
    });

    it('should throw error when embedding generator times out', async () => {
      const store = await setupStoreWithChunks();
      const retriever = createSmallToBigRetriever(store);
      ...
      // The retriever has 10 second timeout, but test has 5 second timeout
      // This test validates that the timeout mechanism exists
      // We use a short timeout in the test to avoid waiting too long
      const timeoutPromise = retriever.getQueryEmbedding('test query');

      // Race between the query and a short timeout
      await expect(
        Promise.race([
          timeoutPromise,
    ...
    it('should use synthetic embedding when no generator set', async () => {
      const store = await setupStoreWithChunks();
      const retriever = createSmallToBigRetriever(store);

      // No generator set - should use synthetic embedding
      const embedding = await retriever.getQueryEmbedding('test query');

      expect(Array.isArray(embedding)).toBe(true);
      expect(embedding.length).toBe(1536); // Synthetic embedding dimension
    });
  });
  ...
  describe('retrieve', () => {
    ...
    // 6.3: Verify retrieval returns relevant chunks not full documents
    it('should return chunks not full documents', async () => {
      const store = await setupStoreWithChunks();
      const retriever = createSmallToBigRetriever(store, {
        similarityThreshold: 0.0,
      ...
      // Each result should be a chunk, not a full document
      expect(results.length).toBeGreaterThan(0);
      expect(results.length).toBeLessThan(100); // Not all chunks

      // Parent chunks should be bounded in size
      for (const result of results) {
        const parentTokens = estimateTokenCount(result.parentChunkContent);
        expect(parentTokens).toBeLessThan(500); // Parent chunks have size limits
      }
    });

    // 6.2: Integration test for retrieval with real embeddings
    it('should work with realistic embeddings', async () => {
      const store = await setupStoreWithRealisticEmbeddings();
      const retriever = createSmallToBigRetriever(store, {
        similarityThreshold: 0.3,
        topK: 5,
      });

      // Set embedding generator that produces similar embeddings for ML queries
      retriever.setEmbeddingGenerator(async (text: string) => {
        if (text.includes('machine learning')) {
          return createRealisticEmbedding(100);
        }
        if (text.includes('deep learning')) {
          return createRealisticEmbedding(101);
        }
        return createRealisticEmbedding(999);
      });

      const results = await retriever.retrieve('machine learning');

      expect(results.length).toBeGreaterThan(0);
      // Should find ML-related chunks with proper similarity calculation
  ...
  describe('fallback', () => {
    ...
    it('should return empty when no fallback results meet threshold', async () => {
      const store = await setupStoreWithChunks();
      const retriever = createSmallToBigRetriever(store, {
        similarityThreshold: 0.99,
        enableFallback: true,
        fallbackThreshold: 0.99, // High threshold - no results will pass
      });

      const results = await retriever.retrieve('completely unrelated xyz');
