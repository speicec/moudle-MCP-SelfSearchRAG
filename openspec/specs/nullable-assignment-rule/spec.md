...
#### Scenario: ESLint 9 flat config compatibility
- **WHEN** ESLint runs with flat config (`eslint.config.js`)
- **THEN** the custom rule SHALL be correctly registered via `plugins` property
...
#### Scenario: typescript-eslint v8 API compatibility
- **WHEN** custom rule imports from `@typescript-eslint/utils` v8
- **THEN** all APIs used (`ESLintUtils.RuleCreator`, `getParserServices`) SHALL work correctly
